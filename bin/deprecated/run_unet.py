...
def train(
    ...
    for _ in t:
        ...
        if (_ + 1) % epoch_lapse == 0:
            ...
            print(
                "Total loss in epoch %f : %f and validation loss : %f" %
...
def plot_examples(unet, datax, datay, num_examples=3):
    ...
    for row_num in range(num_examples):
        ...
        image_arr = unet(torch.from_numpy(
            datax[image_indx:image_indx + 1]).float().cuda()).squeeze(0).detach().cpu().numpy()
        ax[row_num][0].imshow(
            np.transpose(
                datax[image_indx], (1, 2, 0))[
                :, :, 0])
        ax[row_num][1].imshow(np.transpose(image_arr, (1, 2, 0))[:, :, 0])
        ax[row_num][2].imshow(image_arr.argmax(0))
        ax[row_num][3].imshow(
            np.transpose(
                datay[image_indx], (1, 2, 0))[
                :, :, 0])
    plt.show()
