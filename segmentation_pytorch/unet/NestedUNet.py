...
class UNet(nn.Module):
    def __init__(self, dim_in=1, n_classes=8, **args):
        ...
        self.path_1 = VGGBlock(dim_in, nb_filter[0], nb_filter[0])
        self.path_2 = VGGBlock(dim_in, nb_filter[1], nb_filter[1])
        self.path_3 = VGGBlock(dim_in, nb_filter[2], nb_filter[2])
        self.path_4 = VGGBlock(dim_in, nb_filter[3], nb_filter[3])

        self.conv0_0 = VGGBlock(dim_in,
                                nb_filter[0], nb_filter[0])
        self.conv1_0 = VGGBlock(nb_filter[0]*2, nb_filter[1], nb_filter[1])
        self.conv2_0 = VGGBlock(nb_filter[1]*2, nb_filter[2], nb_filter[2])
        self.conv3_0 = VGGBlock(nb_filter[2]*2, nb_filter[3], nb_filter[3])
        self.conv4_0 = VGGBlock(nb_filter[3]*2, nb_filter[4], nb_filter[4])

        self.conv3_1 = VGGBlock(
            nb_filter[3]*2+nb_filter[4], nb_filter[3], nb_filter[3])
        ...
        self.final1 = nn.Conv2d(nb_filter[3], n_classes, kernel_size=1)
        self.final2 = nn.Conv2d(nb_filter[2], n_classes, kernel_size=1)
        self.final3 = nn.Conv2d(nb_filter[1], n_classes, kernel_size=1)
    ...
    def forward(self, input):
        ...
        x2_0 = self.conv2_0(
            torch.cat([self.pool(x1_0), self.path_2(input[2])]))
        x3_0 = self.conv3_0(
            torch.cat([self.pool(x2_0), self.path_3(input[3])]))
        x4_0 = self.conv4_0(
            torch.cat([self.pool(x3_0), self.path_4(input[4])]))
