...
class SegmentationMetric(Metric):
    ...
    def value(self):
        ...
        true_negative = np.sum(conf_matrix) - true_positive - \
...
class FWAVACCMetric(nn.Module):
    ...
    def forward(self, y_pr, y_gt):
        ...
        conf_matrix = confusion_matrix(
            y_pr.view(-1), y_gt.view(-1), self.num_classes, self.normalized)
        if self.ignore_index is not None:
            for index in self.ignore_index:
                conf_matrix[:, index] = 0
                conf_matrix[index, :] = 0
        ...
        iou_score = (true_positive) / (conf_matrix.sum(0) +
                                       conf_matrix.sum(1)-true_positive)

        fwavacc = (freq[freq > 0] * iou_score[freq > 0]).sum()
        if freq.sum() == 0:
            import pdb
            pdb.set_trace()
            return -1
        # import pdb; pdb.set_trace()
        # mean_iou = torch.tensor(np.nanmean(iou_score)) #*IOU
        ...
        # mean_iou = torch.tensor(np.nanmean(iou_score))
        return fwavacc
