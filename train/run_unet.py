...
# 	x_train, y_train = create_dataset(subject_path[:m - 1], width_in, height_in, width_out, height_out, data_indexes,
# 									  mat)
# 	x_val, y_val = create_dataset(subject_path[m - 1:], width_in, height_in, width_out, height_out, data_indexes, mat)
# 	return x_train, y_train, x_val, y_val


def train_step(inputs, labels, optimizer, criterion, model, width_out, height_out):
	optimizer.zero_grad()
...
def train(unet, batch_size, epochs, epoch_lapse, threshold, learning_rate, criterion, optimizer, x_train, y_train,
		  x_val, y_val, width_out, height_out):
...
def plot_examples(unet, datax, datay, num_examples=3):
	...
	for row_num in range(num_examples):
		...
		image_arr = unet(torch.from_numpy(datax[image_indx:image_indx + 1]).float().cuda()).squeeze(
			0).detach().cpu().numpy()
...
def main():
	...
	if sys.argv[1] == 'train':
		train(unet, batch_size, epochs, epoch_lapse, threshold, learning_rate, criterion, optimizer, x_train, y_train,
			  x_val, y_val, width_out, height_out)
		pass
	else:
		if use_gpu:
